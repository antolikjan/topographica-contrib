...
import pylab
import sys
sys.path.append('/home/jan/installs/Theano-eddie/')
import theano
...
class LSCSMNEW(object):
        ...
        def returnRFs(self,K):
            ...
            return rfs,a1,a,x,y,sc,ss,rc,rs
...
def addRPI():
    ...
    #prefix = './LSCSM/RESULTS/LSCSM/NumLgn=9_HLS=0.2_NoEarlyStopping_DOG_Seeds50/'
    prefix = './'
...
def loadLSCSMandAnalyse(): 
    ...
    #prefix = './LSCSM/RESULTS/LSCSM/NumLgn=9_HLS=0.2_NoEarlyStopping/'
    #prefix = './LSCSM/RESULTS/LSCSM/NumLgn=10_HLS=0.2_NoEarlyStopping/'
    prefix = './LSCSM/RESULTS/LSCSM/NumLgn=9_HLS=0.2_NoEarlyStopping_DOG_Seeds50/'
    #prefix = './LSCSM/RESULTS/LSCSM/NumLgn=12_HLS=0.2_NoEarlyStopping_DOG/'
    #prefix = './LSCSM/RESULTS/LSCSM/'
    ...
    for i in xrange(0,len(animals)):
        gallant_pred_val_act.append(numpy.loadtxt('./LSCSM/RESULTS/Gallant/'+gallant_vr_names[i]))
        gallant_pred_act.append(numpy.loadtxt('./LSCSM/RESULTS/Gallant/'+gallant_tr_names[i]))
...
def AnalyzeSTARF():
    ...
    animal_data = ['./LSCSM/RESULTS/LSCSM/NumLgn=9_HLS=0.2_NoEarlyStopping_DOG_Seeds50/2009_11_04_region3.dat','./LSCSM/RESULTS/LSCSM/NumLgn=9_HLS=0.2_NoEarlyStopping_DOG_Seeds50/2009_11_04_region5.dat','./LSCSM/RESULTS/LSCSM/NumLgn=9_HLS=0.2_NoEarlyStopping_DOG_Seeds50/2010_04_22.dat']
...
def loadBestResults():
    ...
    # load locations
    loc = []
    f = file("./LSCSM/Mice/2009_11_04/region3_cell_locations", "r")
    ...
    f = file("./LSCSM/Mice/2009_11_04/region5_cell_locations", "r")
    loc.append([line.split() for line in f])
    for (i,s) in enumerate(loc[-1]):
        loc[-1][i][0] = float(s[0])
        loc[-1][i][1] = float(s[1])
    ...
    f = file("./LSCSM/Mice/2010_04_22/cell_locations", "r")
    loc.append([line.split() for line in f])
    for (i,s) in enumerate(loc[-1]):
        loc[-1][i][0] = float(s[0])
        loc[-1][i][1] = float(s[1])
    ...
    for a in animals:
      ...
      lscsm_x.append(lscsm.returnRFs(K)[3])
      lscsm_y.append(lscsm.returnRFs(K)[4])
      lscsm_ss.append(lscsm.returnRFs(K)[5])
      lscsm_sc.append(lscsm.returnRFs(K)[6])
      lscsm_rs.append(lscsm.returnRFs(K)[7])
      lscsm_rc.append(lscsm.returnRFs(K)[8])
